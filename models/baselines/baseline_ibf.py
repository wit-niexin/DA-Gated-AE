...
Description:
    ...
    本实验遍历多个噪声等级，自动化计算 PSNR, SSIM, ENL, CNR 等指标，并生成定量报告。
...
def run_ibf_experiment():
    """
    主实验流程：读取测试集 -> 传统算法去噪 -> 指标计算 -> 自动汇总报告
    """
    # --- 2. 路径初始化 ---
    ...
    for sigma_val in NOISE_LEVELS:
        ...
        res = {
            ...
            "FPS": f"{1.0 / np.mean(metrics_cache['time']):.1f}",  # 每秒处理张数
            "Params": "0 (Traditional)"
        }
