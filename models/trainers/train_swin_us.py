...
# ==========================================
MODEL_NAME = "swin_us"
BATCH_SIZE = 1          # 物理 Batch Size
ACCUM_ITER = 4          # ✅ 新增：梯度累加步数，实际等效 Batch Size = 1 * 4 = 4
EPOCHS = 100            # Transformer 建议至少 100 轮
LR = 2e-4               
WEIGHT_DECAY = 1e-5     
...
def train():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if avg_loss < best_loss:
            ...
            log_msg = f"⭐ Epoch {epoch}: 新的最佳 Loss: {avg_loss:.6f}"
        else:
    ...
    logger.log_text(f"✨ 训练完成。最优损失: {best_loss:.6f}")
